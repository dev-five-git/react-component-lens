...
use crate::resolver::{FileSystem, NativeFileSystem, resolve_import_with_fs};
...
pub fn analyze_fixture(case_dir: &Path) -> String {
    ...
    let mut usages = analyze_document(&entry_path, &source_text, scope, &FileSystemSourceHost, &fs);
...
pub fn analyze_source_with_host(
    ...
    analyze_source_with_host_and_scope(file_path, source_text, ScopeConfig::default(), host)
}

/// Analyze a single in-memory source file with explicit usage scope.
#[must_use]
pub fn analyze_source_with_host_and_scope(
    file_path: &Path,
    source_text: &str,
    scope: ScopeConfig,
    host: &impl SourceHost,
    ...
    let fs = NativeFileSystem;
    analyze_source_with_host_and_scope_and_fs(file_path, source_text, scope, host, &fs)
}

/// Analyze a single in-memory source file with explicit usage scope and an
/// injected module-resolution filesystem.
#[must_use]
pub fn analyze_source_with_host_and_scope_and_fs<F: FileSystem + Send + Sync + 'static + Clone>(
    file_path: &Path,
    source_text: &str,
    scope: ScopeConfig,
    host: &impl SourceHost,
    ...
    analyze_document(file_path, source_text, scope, host, fs)
}

/// Find a locally declared component's zero-based declaration position as
/// `(line, character)` in UTF-16 code units.
#[must_use]
pub fn find_component_declaration(
    file_path: &Path,
    source_text: &str,
    component_name: &str,
    _host: &impl SourceHost,
) -> Option<(u32, u32)> {
    let analysis = parse_file_analysis(file_path, source_text);
    let component = analysis.local_components.get(component_name)?;
    let range = component.ranges.first()?;
    let mapper = Utf16Mapper::new(source_text);
    Some(mapper.line_and_character_for_utf16(range.start))
}
...
fn analyze_document(
    ...
    scope: ScopeConfig,
    host: &impl SourceHost,
    fs: &(impl FileSystem + 'static + Clone),
) -> Vec<Usage> {
    let analysis = parse_file_analysis(file_path, source_text);
    ...
    if (scope.element || scope.import) && !analysis.imports.is_empty() {
        ...
        for (lookup_name, entry) in &analysis.imports {
            ...
            if let Some(resolved_file_path) =
                resolve_import_with_fs(file_path, &entry.source, fs.clone())
            {
            ...
            if export_name == "*" || file_info.component_names.contains(export_name) {
                ...
                let re_export_resolution = if let Some((source, source_name)) = re_export
                    && let Some(target_path) =
                        resolve_import_with_fs(file_path, &source, fs.clone())
                {
...
mod tests {
    ...
    impl TempProject {
        fn new() -> Self {
            ...
            let root =
                std::env::temp_dir().join(format!("rcl-core-analyzer-{}-{id}", std::process::id()));
            if root.exists() {
    ...
    #[test]
    fn mixed_component_forms_cover_tracking_import_and_variable_branches() {
        let project = TempProject::new();
        let source = "
            import DefaultButton from './missing';
            import * as Icons from './missing';
            import { Card } from './missing';
            const { Ignored } = props;
            const lower = () => null;
            let Missing;
            export async function AsyncThing(){ return <Card/>; }
            export function App(){ function helper(){ 'use server'; } return <Button/>; }
            export const Button = () => <Card/>;
        ";
        let entry = project.write("entry.tsx", source);

        let usages = analyze_temp_source(&entry, source);
        let server_names = names_with_kind(&usages, Kind::Server);

        assert!(server_names.contains("App"));
        assert!(server_names.contains("AsyncThing"));
        assert!(server_names.contains("Button"));
        assert!(!server_names.contains("lower"));
        assert!(!server_names.contains("Missing"));
    }
